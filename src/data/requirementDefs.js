...
export const REQUIREMENT_DEFS = {
    ...
    resourceUnderMaxBy: {
        create: (resource, value) => ({ type: "resourceUnderMaxBy", resource, value }),
        check(game, r) {
    ...
    resourceLessThan: {
        create: (resource, value) => ({ type: "resourceLessThan", resource, value }),
        check: (game, r) => game.resources[r.resource].current < resolve(game, r.value),
    },

    hasCondition: {
        create: (condition, min_duration = null) => ({ type: "hasCondition", condition, min_duration }),
        check(game, r) {
            if (!(r.condition in game.activeConditions)) return false;
            if (r.min_duration == null) return true;
            return resolve(game, r.min_duration) <= game.activeConditions[r.condition].duration;
        },
    },
